...
`;
const List = styled.ul<{ isOver: boolean }>`
  display: grid;
...
`;
const Overlay = styled.div<{ isProfile: boolean }>`
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
...
function Actors({ id, isMovie = false }: Props) {
          ...
          {actors.map((actor, index) => (
            <Link to={`/actor/${actor.id}`} key={index}>
              <Item>
                ...
                <Profile
                  profileUrl={
                    actor.profile_path ? `https://image.tmdb.org/t/p/w200${actor.profile_path}` : noPosterImage
                  }
                />
